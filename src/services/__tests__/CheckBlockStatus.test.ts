...
import { checkBlockStatus } from '../CheckBlockStatus';
...
describe('blockUser', () => {
  ...
  it('should send a POST request with correct data and return response', async () => {
    ...
    expect(mockedFetch).toHaveBeenCalledWith(`${API_URL}/api/users/block-status`, {
      method: 'POST',
      headers: {
  ...
  it('should throw if fetch fails', async () => {
    ...
    await expect(checkBlockStatus(payload)).rejects.toThrow('Network Error');
  });
});
