...
import { getAllChats, getMissedChats } from '../GetAllChats';
...
describe('Tests for get Missed chats function', () => {
  const mockUserDetails = {
    id: '14c9dda3-9a94-4f87-8b16-1669c0e818c3',
    firstName: 'Sujji',
    lastName: 'Medikonda',
  ...
  const lastSyncedTime = '2025-05-26T16:05:00.000Z';
  const mockChats = [
    {
      chatId: 'f1a8e140-0a40-4050-a77e-8624fb5603e0',
      contactName: 'receiver B',
  ...
  test('Fetch all contacts', async () => {
    ...
    mockedFetch.mockResolvedValueOnce({
      status: 200,
      json: jest.fn().mockResolvedValue({data:mockChats}),
    });

    const result = await getMissedChats(lastSyncedTime);

    await waitFor(() => {
      expect(EncryptedStorage.getItem).toHaveBeenCalledTimes(2);
      expect(fetch).toHaveBeenCalledWith(
        expect.stringContaining(`${API_URL}/api/sync`),
        expect.objectContaining({
          method: 'POST',
          headers: expect.objectContaining({
            authorization: 'Bearer dummy-token',
          ...
          body: JSON.stringify({
            userPhoneNumber: '+919248434816',
            lastSyncedAt: '2025-05-26T16:05:00.000Z',
          }),
        }),
      );
      expect(result.data).toEqual(mockChats);
