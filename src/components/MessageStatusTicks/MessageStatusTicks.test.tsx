...
jest.mock('../../assets/pendingDark.png', () => 'pendingImageMock');
...
describe('MessageStatusTicks Test Suit', () => {
  ...
  test('should render the pending image', () => {
    const {getByA11yHint} = render(<MessageStatusTicks status="pending" />);
    const image = getByA11yHint('Message status is pending');
    expect(image.props.source).toBe('pendingImageMock');
  });
